...
```{r}
 ...
 plot(daily$date, daily$dailysum, type = "h", main = "Total steps per day", ylab = "Steps", xlab = "")
...
```{r}
 averageactivity <- ddply(activity, .(interval), summarise, intervalmean=mean(steps, na.rm = TRUE))
 
xyplot(intervalmean ~ interval, data = averageactivity, type = "l", xlab = "Time of Day (in minutes)", ylab = "Average Steps")
...
for(i in 1:length(filledactivity$steps)){
  if (is.na(filledactivity$steps[i])){
      filledactivity$steps[i]<- averageactivity$intervalmean[averageactivity$interval == filledactivity$interval[i]]
  }
}
```
...
```{r}
 filleddaily <- ddply(filledactivity, .(date), summarise, dailysum=sum(steps, na.rm = TRUE))
 plot(filleddaily$date, filleddaily$dailysum, type = "h", main = "Total steps per day (NA replaced)", ylab = "Steps", xlab = "")
...
for(i in 1:length(filledactivity$steps)){
  if (weekdays(as.Date(filledactivity$date[i])) == "Saturday" | weekdays(as.Date(filledactivity$date[i])) == "Sunday"){
...
```{r}
xyplot(steps ~ interval | weekday, data = daytypeaverage, type = "l", layout = c(1,2), xlab = "Time of Day (in minutes)", ylab = "Average Steps")
